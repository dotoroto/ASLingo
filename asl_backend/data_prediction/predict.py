...
def np_from_base64_jpeg(data_url_or_b64: str) -> np.ndarray:
    """Convert base64 (optionally data URL) JPEG to RGB numpy image."""
...
def extract_landmarks_rgb(np_rgb: np.ndarray) -> Optional[np.ndarray]:
    """Return flat (63,) landmarks if a hand is found; else None."""
    # MediaPipe expects RGB, so we can use the image directly
    results = mp_hands.process(np_rgb)
